			...
			<view class="driver_list" v-for="(item,index) in source" :key="index">
				<view class="driver_c">
					<view class="left">
						...
						<text style="margin-left:30rpx;">{{item.user.userinformation.realname}}</text>
					</view>
				...
				<view class="driver_t">
					<view class="position">{{item.user.userinformation.car_number}}</view>
					<view class="time">{{item.car_length}}米-平板 / 高栏-{{item.weight}}吨</view>
				</view>
				<view class="driver_b">
					<view class="material">类型:{{item.car_type}}</view>
					<!-- <view class="long">4分钟</view> -->
				</view>
				<view class="driver_b2">
					<view class="category">长走路线:{{item.city_name}}-{{item.end_city_name}}</view>
				</view>
				<slot name="car"></slot>
			</view>
		</view>
